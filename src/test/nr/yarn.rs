...
mod bun {
    ...
    #[test]
    fn empty() {
        let (agent, args) = parse_nr(Agent::Yarn, Vec::new());
        assert_eq!(agent, "yarn");
        assert_eq!(args, ["run", "start"]);
    }
    #[test]
    fn if_present() {
        let (agent, args) = parse_nr(Agent::Yarn, vec!["test".into(), "--if-present".into()]);
        assert_eq!(agent, "yarn");
        assert_eq!(args, ["run", "--if-present", "test"]);
    }
    #[test]
    fn script() {
        let (agent, args) = parse_nr(Agent::Yarn, vec!["dev".into()]);
        assert_eq!(agent, "yarn");
    ...
    #[test]
    fn colon() {
        let (agent, args) = parse_nr(Agent::Yarn, vec!["build:dev".into()]);
        assert_eq!(agent, "yarn");
